    ...
    for rel, meta in new.items():
        if rel not in old:
            ...
            old[rel]["hash"] == "" and meta["hash"] == "" and (
                ...
                abs((old[rel].get("mtime") or 0.0) - (meta.get("mtime") or 0.0)) > 2.0
            )
        ):
            changes.append({
                "type": "modified",
            ...
            if _use_robocopy:
                ...
                if not files_to_copy:
                    ...
                    _rc_dir_ok, _rc_dir_files, _rc_dir_bytes = _robocopy_dir(
                        ...
                        files_to_copy=files_to_copy if (incremental and previous_snapshot and files_to_copy) else None,
                    ...
                    if _rc_dir_ok:
                        ...
                        # Skip post-copy hashing — reading every byte back over the network
                        # just to compute SHA-256 would negate the speed benefit of the
                        # robocopy fast-path for large SMB shares.
                        # Store "" for now (same as seed-skip); the next incremental run
                        # will backfill real hashes inline as it compares files.
                        for entry in files_to_copy:
                            _rel = entry["path"].replace("/", "\\")
                            _files_written_hashes[_rel] = ""
                            # Keep whatever hash the snapshot already has (from source scan)
