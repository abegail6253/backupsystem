...
try:
    ...
    from watcher import WatcherManager, register_history_persist_suppressor, _get_net_session_enum_fn
...
def _query_smb_audit(host: str, filepath: str, event_type: str,
                     timestamp_iso: str,
                     smb_audit_cfg: dict | None = None) -> dict:
    """
    ...
    if _win_user and _win_pass and host:
        try:
            ...
            _USE_IPC = getattr(win32netcon, "USE_IPC", 3)
            _use_info = {
                "remote":     f"\\\\{host}\\IPC$",
                "username":   _win_user,
                "password":   _win_pass,
            ...
            try:
                win32net.NetUseAdd(None, 1, _use_info)
                _net_use_ok = True
                _qna.info(f"[_query_smb_audit] Strategy1: IPC$ NetUseAdd OK for {host!r} user={_win_user!r}")
            except Exception as _ipc_err:
                _qna.info(f"[_query_smb_audit] Strategy1: IPC$ NetUseAdd FAILED for {host!r}: {_ipc_err!r} (may already be connected — proceeding anyway)")
        except ImportError:
            _qna.info("[_query_smb_audit] Strategy1: win32net not available (pywin32 not installed) — cannot pre-auth IPC$")
    else:
        _qna.info(f"[_query_smb_audit] Strategy1: skipping IPC$ auth — no SMB credentials configured. FIX: add Windows PC credentials in watch settings.")
    ...
    if host and not result.get("user"):
        try:
            ...
            if _s1b_handle:
                    ...
                    while _s1b_read_count < _s1b_max_events:
                        ...
                        for _ev in _s1b_events:
                            ...
                            try:
                                ...
                                _diff = abs((_ev_dt - event_dt.replace(tzinfo=None)).total_seconds())
                                if _diff > 60:
                                    # Events are backwards (newest first); if we've gone
                                    # more than 5 min past the window, stop reading
                                    if (_ev_dt - event_dt.replace(tzinfo=None)).total_seconds() < -300:
                                        break
                                    continue
                            except Exception:
                                continue
                            ...
                            if not _s1b_subject or _s1b_subject.endswith("$") or _s1b_subject.upper() in ("SYSTEM", "LOCAL SERVICE", "NETWORK SERVICE", ""):
                                continue
                            # Match filename
                            if _s1b_fname and _s1b_fname not in _s1b_objname.lower():
                                # Try loose match via all string inserts
                                _all_str = " ".join(str(s) for s in _strs).lower()
                                if _s1b_fname not in _all_str:
                                    continue
                            ...
                            _qna.info(
                                ...
                                f"object={_s1b_objname!r} time_diff={_diff:.1f}s "
                ...
                if not _s1b_found:
                    _qna.info(
                        f"[_query_smb_audit] Strategy1b: no matching Security log event found "
                        f"(read {_s1b_read_count} events). "
                        f"If SACL auditing is not configured on the shared folder this is expected. "
                        f"FIX: right-click the shared folder on {host!r} → Properties → Security → "
    ...
    # Early-exit if Strategy1b already found a user
    if result.get("user"):
        _qna.info(
            f"[_query_smb_audit] Strategy1b resolved attribution — "
        ...
        if _nse_fn is not None:
            ...
            for _s4_try in range(1, _max_s4_tries + 1):
                ...
                for sess in sessions:
                    ...
                    num_open = sess.get("num_opens", "?")
                    idle_sec = sess.get("num_users", "?")
                    try:
                    ...
                    if _verdict == "MATCH":
                        result["user"]    = uname
                        result["machine"] = client
                        result["ip"]      = client_ip
                        _qna.info(
                            f"[_query_smb_audit] Strategy4: MATCH → user={uname!r} "
                            f"machine={client!r} ip={client_ip!r} "
            ...
            if _s6_fn is None:
                ...
                for _s6_try in range(1, _s6_max + 1):
                    ...
                    for _s6_sess in _s6_sessions_raw:
                        ...
                        _s6_uname  = _s6_sess.get("username") or ""
                        _s6_opens  = _s6_sess.get("num_opens", "?")
                        try:
...
def _get_editor_info(filepath: str, detection_source: str = "",
    ...
    if is_unc and remote_host:
        ...
        _gei.info(f"[_get_editor_info] Steps2-4: calling _query_smb_audit for {remote_host!r}")
...
class EditWatchDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        # ── Force robocopy checkbox (before compression so it's always visible) ──
        ...
        form.addRow("", self.force_robocopy_check)
        ...
        # Auto-detect same-host UNC on dest change; also honour manual toggle
        ...
        # Apply immediately on dialog open
        ...
        layout.addWidget(self._nas_creds_toggle)
        ...
        layout.addWidget(self._nas_creds_widget)

        # Pre-populate from existing watch config
    ...
    def _test_nas_credentials(self):
        ...
        def _do_test():
            try:
                import win32net, win32netcon
                # ── Force-disconnect any cached OS session then reconnect ────
                # Windows reuses existing SMB sessions even when a different
                # password is supplied (error 1219, silently ignored before).
                # NetUseDel flushes the cache so NetUseAdd authenticates fresh.
                _ipc = f"\\\\{_host}\\IPC$"
                try:
                    win32net.NetUseDel(None, _ipc, 0)
                except Exception:
                    pass   # no existing session — that's fine
                _USE_IPC = getattr(win32netcon, "USE_IPC", 3)
                _use_info = {
                    "remote":     _ipc,
                    "username":   _user,
                ...
                try:
                    ...
                    return False, f"❌  Auth failed (wrong password or username): {_e}"
                try:
                    _tc_nse_fn = _get_net_session_enum_fn("[test_connection] ")
                    if _tc_nse_fn is not None:
                        sessions = _tc_nse_fn(_host)
                        n = len(sessions or [])
                        return True, f"✅  Connected — {n} active session(s) visible"
                    else:
                        return True, "✅  Authenticated (session query unavailable on this pywin32 build)"
                except Exception as _e2:
                    return True, f"✅  Authenticated (session query limited: {_e2})"
            except ImportError:
                return False, "pywin32 not available on this machine"
            except Exception as _e:
...
class AddWatchDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        self.dest_input.setPlaceholderText(
            f"Leave blank to use global ({global_dest})" if global_dest
            else "Leave blank to use global destination"
        )
        ...
        _nas_hdr = QLabel("WHO-DID-IT TRACKING  (optional)")
        _nas_hdr.setStyleSheet(_sec_label_style)
        _nas_inner.addRow("", _nas_hdr)
        ...
        def _do_test():
            ...
            try:
                ...
                try:
                    ...
                    ok = False
                    msg = f"❌  Auth failed (wrong password or username): {_e}"
                    return ok, msg
                # Auth succeeded — try NetSessionEnum to confirm admin access
                try:
                    _tc_nse_fn2 = _get_net_session_enum_fn("[test_connection2] ")
                    if _tc_nse_fn2 is not None:
                        sessions = _tc_nse_fn2(_host)
                        ok = True
                        n = len(sessions or [])
                        msg = f"✅  Connected — {n} active session(s) visible"
                    else:
                        ok = True
                        msg = "✅  Authenticated (session query unavailable on this pywin32 build)"
                except Exception as _e2:
                    # Connected but can't enum sessions — still a partial success
                    ok = True
                    msg = f"✅  Authenticated (session query limited: {_e2})"
            except ImportError:
                ok = False
                msg = "pywin32 not available on this machine"
            except Exception as _e:
                ok = False
                msg = f"Error: {_e}"
            return ok, msg
...
class AdminPanel(QDialog):
    ...
    def _build_ui(self):
        ...
        self.dest_input = QLineEdit()
        self.dest_input.setPlaceholderText("C:\\BackupData  or  \\\\server\\share\\backups")
    ...
    def set_progress(self, current: int, total: int, fname: str = "", elapsed: float = 0.0, is_scanning: bool = False, bytes_done: int = 0, total_bytes: int = 0):
        ...
        if is_scanning:
            ...
            if (total == 0 and not use_bytes) or unknown_total:
                ...
                _is_nas_copy  = fname.startswith("\x00nas_copy\x00")
                if _is_nas_copy:
                    ...
                    if bytes_done > 0:
                        ...
                        _nas_fname = fname[len("\x00nas_copy\x00"):]
                ...
                if speed_bps > 0:
                    ...
                    if use_bytes and total_bytes > bytes_done and speed_bps > 0:
                        ...
                        _eta_ready = _speed_window_duration >= 20.0 and _pct_done >= 0.01
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        _POST_BACKUP_SUPPRESS_SECS = 60      # class-level constant (accessed as self._POST_BACKUP_SUPPRESS_SECS)
    ...
    def _on_file_change_inner(self, watch_id: str, entry: dict):
        ...
        if _is_dest:
            ...
            # If the same (watch_id, path, type) was already recorded within 60 s,
            ...
            if _last_seen is not None and (_now_mono - _last_seen) < _DEDUP_WINDOW:
                ...
                if _etype == "deleted":
                    ...
                    for _stored in reversed(self._history_log):
                        ...
                        if not (_path_match and _type_match):
                            ...
                            continue
                        # Found the matching stored deletion entry
                    ...
                    if not _dedup_matched_stored:
                        ...
                        _wait_deadline = _dedup_wait_time.monotonic() + 5.0
                        ...
                        logger.info(
                            ...
                            f"waiting up to 5s for it to append "
                        ...
                        if _dedup_found_late:
                            ...
                            logger.info(
                                f"[desktop._on_file_change] DEDUP late-match: "
                                f"timed out after 5s — first-detector entry never appeared; "
                ...
                for _stored2 in reversed(self._history_log):
                    if (_stored2.get("path", "").lower() == _path_lower_ls
                        ...
                        _stored2["editor_machine"]      = f"PC ({_nas_ip})" if _nas_ip else "PC"
                        _stored2["editor_ip"]           = _nas_ip
        ...
        # ── Debug: explain Unknown attribution for Windows PC same-host targets ─
        if entry.get("editor_user") == "Unknown" and entry.get("from_destination"):
            ...
            if _ehost:
                ...
                _logging.getLogger(__name__).warning(
                    f"[desktop._on_file_change] ATTRIBUTION=Unknown for dest event "
...
class HistoryWindow(QDialog):
    ...
    def _populate_changes(self, entries: list):
        ...
        for i, e in enumerate(reversed(entries)):
            ...
            # Use amber/orange for unresolved identity, normal colors otherwise
            _user_color    = "#f59e0b" if _unknown else "#60a5fa"
            _machine_color = "#f59e0b" if _unknown else "#a78bfa"
            _ip_color      = "#9ca3af" if _unknown else "#34d399"
            _user_display    = f"⚠ {user}" if _unknown else user
            _machine_display = machine
            _user_item = _item(_user_display, _user_color)
            if _unknown:
                _user_item.setToolTip(
                    "Identity could not be determined.\n"
                    "The PC knows who did this — to see it:\n"
                    "1. Edit this watch → expand 'SMB Credentials'\n"
                    ""
                    "3. In DSM: Log Center → Log Sending → enable File transfer log"
                )
    ...
    def append_entry(self, entry: dict):
        ...
        # ── Attribution column: colour Unknown distinctly and add tooltip ────
        _user_text    = entry.get("editor_user", "")
        _is_unknown   = entry.get("attribution_unknown", False) or _user_text == "Unknown"
        _user_clr     = "#f59e0b" if _is_unknown else "#60a5fa"   # amber for Unknown
    ...
    def update_entry(self, entry: dict):
        ...
        for row in range(self.table.rowCount()):
            ...
            if (path_item.text().lower() == path_lower
                ...
                self.table.item(row, 4).setText(entry.get("editor_user",    ""))
                self.table.item(row, 5).setText(entry.get("editor_machine", ""))
                self.table.item(row, 6).setText(entry.get("editor_ip",      ""))
                break
