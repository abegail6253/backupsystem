...
**SMB**
```json
"dest_smb": { "server": "192.168.1.100", "share": "backups", "username": "user", "password": "pass", "remote_path": "" }
```
...
Set `dest_type` to `rclone` and the remote name to any remote shown by `rclone listremotes`. rclone supports 70+ providers including network storage, SFTP, Storj, Mega, pCloud, and others — configure the remote once in rclone, then point BackupSys at it.
