...
class IntegrityScheduler:
    ...
    def check_disk_space(self):
        ...
        try:
            import shutil, os
            # Walk up to the nearest existing ancestor so we still get a
            # meaningful disk-usage figure even when the destination
            # subfolder hasn't been created yet.  Without this,
            # shutil.disk_usage() raises [WinError 3] / FileNotFoundError
            # for a path like D:\backups\watches\test that doesn't exist yet.
            probe = destination
            while probe and not os.path.exists(probe):
                parent = os.path.dirname(probe)
                if parent == probe:   # reached the drive root and it's gone too
                    break
                probe = parent

            if not probe or not os.path.exists(probe):
                logger.warning(
                    "check_disk_space: destination and all ancestors missing: %s",
                    destination,
                )
                return
